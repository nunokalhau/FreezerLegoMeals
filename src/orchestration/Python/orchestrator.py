...
class Orchestrator:
    ...
    def execute(self, context) -> OrchestratorResult:
        self.logger.info("Orchestrator started for correlation %s", context.correlation_id)
        ...
        if agent is None:
            ...
            return OrchestratorResult(error, "none", [], [], ["Assistant", "Orchestrator", "NoAgent"], 0.0, [error], context.messages_to_persist)

        self.logger.info("Orchestrator selected %s for correlation %s", agent.name, context.correlation_id)
