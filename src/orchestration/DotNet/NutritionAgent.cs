...
public sealed class NutritionAgent : IAgent
{
    ...
    public Task<OrchestratorResult> ExecuteAsync(OrchestratorContext context, CancellationToken cancellationToken = default) =>
        Task.FromResult(new OrchestratorResult("NutritionAgent is not active yet.", Name, [], [], ["Assistant", "AssistantOrchestrator", Name], TimeSpan.Zero, ["Agent is not active."], context.MessagesToPersist));
}